...
def analyze_image_vision(image_bytes: bytes, mime_type: str = "image/png") -> dict[str, Any]:
    ...
    Also visually identifies phone model and source app from image content.
    """
    default_res = {
        "text": "", "is_ai": False, "ai_reason": "", "ai_confidence": "Low",
        "device_hint": "Unknown", "source_app": "Unknown"
    }
    if not client:
        return default_res
    ...
    for mod_name in models_to_try:
        try:
            ...
            prompt = (
                "Analyze this image carefully for four tasks:\n"
                ...
                "3. Identify the likely phone brand/model that took this photo based on: image style, UI overlay, watermarks, camera UI patterns, aspect ratio, font style, notification bar style, or any visual clues visible in the image. Even if EXIF is stripped, make a best-guess from visual evidence. Examples: 'iPhone 15 Pro (Apple)', 'Samsung Galaxy S24', 'Xiaomi 14', 'OPPO Find X7', 'Unknown Android'.\n"
                "4. Identify the source application this image came from based on: UI elements, watermarks, icons, overlays, layout, color scheme, timestamps. Examples: 'LINE', 'Instagram', 'Facebook', 'TikTok', 'Twitter/X', 'WhatsApp', 'Screenshot (iOS)', 'Screenshot (Android)', 'Camera Roll', 'PromptPay / QR App', 'Banking App', 'Unknown'.\n\n"
                "CRITICAL REQUIREMENTS:\n"
                "- 'ai_reason' MUST BE WRITTEN IN THAI LANGUAGE (ภาษาไทย).\n"
                "- 'ai_reason' MUST BE VERY SHORT (max 1-2 sentences, NO bullet points, under 25 words).\n"
                "- 'device_hint' should be a short English string like 'iPhone 15 Pro (Apple)' or 'Samsung Galaxy S24 (Android)'.\n"
                "- 'source_app' should be a short English string like 'LINE', 'Instagram', 'Screenshot (iOS)', 'Camera Roll'.\n"
                "- If you cannot determine device_hint or source_app, return 'Unknown'.\n\n"
                "Return output in strict valid JSON format:\n"
                "{\n"
                '  "text": "extracted text verbatim",\n'
                '  "is_ai": true/false,\n'
                '  "ai_reason": "คำอธิบายภาษาไทยแบบสั้นกระชับ ห้ามใช้หัวข้อย่อย",\n'
                '  "ai_confidence": "High/Medium/Low",\n'
                '  "device_hint": "phone brand and model based on visual clues",\n'
                '  "source_app": "app or platform this image came from"\n'
                "}"
            )
            ...
            if res and res.text:
                ...
                return {
                    ...
                    "device_hint": str(data.get("device_hint", "Unknown")),
                    "source_app": str(data.get("source_app", "Unknown")),
                }
